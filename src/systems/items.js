...
export function canPurchaseItem(itemId) {
  ...
  if (item.id === "split_shot" && !state.inventory?.weaponSlots.some((slot) => ["ice", "missile", "boomerang", "drone", "prism_railgun", "void_singularity"].includes(slot.id))) return { ok: false, reason: "需要至少一把投射物武器" };
  return { ok: true };
}
...
function applySplitShot(player) {
  const slots = (state.inventory?.weaponSlots || []).filter((slot) => ["ice", "missile", "boomerang", "drone", "prism_railgun", "void_singularity"].includes(slot.id));
